...
export const DrinkMenu: React.FC = () => {
  ...
  const filterMenu = useMemo(() => {
    ...
    return width > breakpoint ? filterArray : filterArray.slice(0, 4 * currentPage);
  ...
  return (
    <S.DrinkMenu>
      ...
      <FlexContainer wrap={'wrap'} justify={'center'}>
        ...
        <S.RefreshWrap>
          <S.Refresh as={'button'} onClick={() => setCurrenPage((prevState) => prevState + 1)}>
            <Icon iconId={'refresh'} height={'24'} width={'24'} viewBox={'0 0 24 24'} />
          </S.Refresh>
        </S.RefreshWrap>
      </FlexContainer>
      <ModalCard menu={filterId} onToggle={onToggle} isOpen={isOpen} />
    </S.DrinkMenu>
  );
};
